...
export default function IntakePage() {
  ...
  return <><TopNav/><Shell><PageTitle title="شات التأهيل" subtitle="سؤال واحد في كل مرة. لا نذكر سعرًا ولا نطلب دفعًا قبل مراجعة الفريق." />
    <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      <Card><div className="mb-4 flex items-center justify-between"><span className="font-bold">التقدم</span><Badge>{Math.min(step+1, steps.length)} / {steps.length}</Badge></div><div className="h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-teal-700 transition-all" style={{width:`${((step+1)/steps.length)*100}%`}} /></div><p className="mt-4 text-sm leading-7 text-slate-600">الهدف ليس أخذ قرار نهائي، بل تصنيف مبدئي واضح وتحويل العميل إلى حالة للفريق.</p></Card>
