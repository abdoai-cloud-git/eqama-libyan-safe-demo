...
export default function CaseDetailPage() {
  ...
  return <><TopNav/><Shell><Link href="/admin" className="mb-4 inline-flex items-center gap-2 text-sm font-bold text-teal-800"><ArrowRight size={16}/> رجوع للوحة</Link><PageTitle title={`${item.fullName} — ${item.id}`} subtitle="ملف الحالة يربط بيانات التأهيل، سبب التصنيف، الخطوة القادمة، الدفع الكاش، والملاحظات الداخلية." />
    <div className="grid gap-6 lg:grid-cols-[1fr_.9fr]">
      ...
      <Card><h2 className="mb-3 text-lg font-black">سجل التحديثات والملاحظات</h2><div className="flex gap-2"><input className="field" value={note} onChange={(e)=>setNote(e.target.value)} placeholder="أضف ملاحظة داخلية"/><button className="btn-primary" onClick={()=>{ if(note.trim()){ addNote(item.id,note); setNote(''); } }}><Plus size={16}/></button></div><ul className="mt-4 space-y-2">{item.internalNotes.map((n,i)=><li key={i} className="rounded-2xl bg-slate-50 p-3 text-sm leading-6">{n}</li>)}</ul></Card></div>
      <div className="space-y-6"><Card><h2 className="mb-3 text-lg font-black">أزرار الديمو</h2><div className="grid gap-2">{statuses.map(s=><button key={s} className="btn-secondary justify-center" onClick={()=>updateCaseStatus(item.id,s)}>نقل إلى: {s}</button>)}</div></Card><Card><h2 className="mb-3 text-lg font-black">الدفع الكاش</h2><p className="mb-3 text-sm leading-7 text-slate-600">قاعدة الديمو: لا يتم طلب الدفع إلا بعد مراجعة الحالة وقبولها مبدئيًا.</p><div className="grid gap-2">{payments.map(p=><button key={p} className="btn-secondary justify-center" onClick={()=>updatePayment(item.id,p)}>{p}</button>)}</div></Card><Card><h2 className="mb-3 text-lg font-black">بعد اكتمال الإقامة</h2><button className="btn-primary w-full justify-center" onClick={()=>convertToWorker(item.id)}>تحويل إلى ملف عامل</button><Link className="mt-3 block text-center text-sm font-bold text-teal-800" href="/admin/workers">مشاهدة ملفات العمال</Link></Card></div>
    </div>
  </Shell></>;
}
