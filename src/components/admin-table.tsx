...
export function CasesTable() {
  ...
  return <Card>
    <div className="mb-4 grid gap-3 md:grid-cols-[1fr_240px]"><label className="relative"><Search className="absolute right-3 top-3 h-5 w-5 text-slate-400"/><input className="field pr-10" placeholder="بحث بالاسم أو الهاتف أو الجنسية أو رقم الحالة" value={query} onChange={(e)=>setQuery(e.target.value)}/></label><select className="field" value={status} onChange={(e)=>setStatus(e.target.value)}>{statuses.map(s => <option key={s} value={s}>{s === 'all' ? 'كل الحالات' : s}</option>)}</select></div>
